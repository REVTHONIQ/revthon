...
def main_menu():
    text = f"**- [سـورس بودي](https://t.me/BodyThon)\
        \n المسـاعد\
        \n\
        \n◽ بودي لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("bodyiq"):
            buttons = [
                (
                    Button.inline("الحـالة ⚒️", data="stats"),
                    Button.url(" Bodython UsᴇʀBoᴛ", "https://t.me/Bodython"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./bodython", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس بودي 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./bodython", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس بودي 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./bodython", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس بودي 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© bodython Help",
                description="Help menu for bodython",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/Bodython"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/Bodython1",
                ),
            )
        ]
        markup = event.client.build_reply_markup(buttons)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت بودي 🧸♥](https://t.me/Bodython)**\
            \n\
            \n❤ بودي هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/Bodython1)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
...
@bodyiq.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@bodyiq.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@bodyiq.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@bodyiq.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
