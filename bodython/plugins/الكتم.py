...
@bodyiq.on(admin_cmd(pattern=f"كتم(?:\s|$)([\s\S]*)"))
async def mutebody(event):
    if event.is_private:
...
@bodyiq.on(admin_cmd(pattern=f"الغاء كتم(?:\s|$)([\s\S]*)"))
async def unmutebody(event):
    if event.is_private:
        replied_user = await event.client.get_entity(event.chat_id)
