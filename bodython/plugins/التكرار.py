...
async def spam_function(event, bodython, bodyiq, sleeptimem, sleeptimet, DelaySpam=False):

    counter = int(bodyiq[0])
    if len(bodyiq) == 2:
        spam_message = str(bodyiq[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            if event.reply_to_msg_id:
                await bodython.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and bodython.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            bodython = await event.client.send_file(
                event.chat_id, bodython, caption=bodython.text
            )
            await _catutils.unsavegif(event, bodython)
        ...
        if BOTLOG:
            ...
            bodython = await event.client.send_file(BOTLOG_CHATID, bodython)
            await _catutils.unsavegif(event, bodython)
        return
    elif event.reply_to_msg_id and bodython.text:
        spam_message = bodython.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
@bodyiq.ar_cmd(pattern="كرر (.*)")
async def spammer(event):
    bodython = await event.get_reply_message()
    bodyiq = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    try:
        counter = int(bodyiq[0])
    except Exception:
    ...
    await spam_function(event, bodython, bodyiq, sleeptimem, sleeptimet)

@bodyiq.on(admin_cmd(pattern="(مكرر|مؤقت|موقت)"))
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, bodyiq, sleeptimem, sleeptimet, DelaySpam=True)


@bodyiq.ar_cmd(pattern="تكرار الملصق$")
...
@bodyiq.ar_cmd(pattern="ايقاف التكرار ?(.*)")
