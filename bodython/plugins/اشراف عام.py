...
from bodython import CMD_HELP, bodyiq
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @bodyThon", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@bodyiq.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(bodython):
    dc = razan = bodython
    i = 0
    sender = await dc.get_sender()
    me = await bodython.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await bodython.get_chat()
    if bodython.is_private:
        user = bodython.chat
        rank = bodython.pattern_match.group(1)
    else:
        bodython.chat.title
    try:
        user, rank = await get_full_user(bodython)
    except:
        pass
    if me == user:
       bodyiq = await razan.edit("▾∮ لا استطيع رفع نفسي 🧸🤍،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await bodython.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await bodython.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
...
@bodyiq.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(bodython):
    dc = razan = bodython
    i = 0
    sender = await dc.get_sender()
    ...
    try:
        ...
        pass
    if me == user:
       bodyiq = await razan.edit("▾∮ لا استطيع تنزيل نفسي 🧸🤍")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
