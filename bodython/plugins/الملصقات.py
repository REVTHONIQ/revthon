...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"bodython_{userid}_{pack}_anim"
    elif is_video:
        return f"bodython_{userid}_{pack}_vid"
    return f"bodython_{userid}_{pack}"
...
@bodyiq.on(admin_cmd(pattern="حزمة"))
async def bodythonpkg(_):
    body = await _.get_reply_message()
    if not body:
    ...
    _id = body.media.document.attributes[1].stickerset.id
    _hash = body.media.document.attributes[1].stickerset.access_hash
    ...
    await edit_or_reply(
        _, f"**- تم اخذ الحزمه بنجاح ✓ \nالحزمه  → [اضغط هنا](https://t.me/addstickers/{bodython_body.set.short_name})**")
