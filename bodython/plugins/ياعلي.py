    ...
    if gvarstatus ("subprivate"):
        if event.is_private:
            try:
                ...
                ch = gvarstatus ("pchan")
                if not ch:
                    return await bodyiq.tgbot.send_message(BOTLOG_CHATID, "** انت لم تضع قناة الاشتراك الاجباري قم بوضعها**")
                try:
                    ch = int(ch)
                except BaseException as r:
                    return await bodyiq.tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ \n{r}**")
                url = f"https://api.telegram.org/bot{tok}/getchatmember?chat_id={ch}&user_id={idd}"
                req = requests.get(url)
                reqt = req.text
                if "chat not found" in reqt:
                    mb = await bodyiq.tgbot.get_me()
                    mb = mb.username
                    await bodyiq.tgbot.send_message(BOTLOG_CHATID, f"**البوت الخاص بك @{mb} ليس في قناة الاشتراك الاجباري**")
                    return
                if "bot was kicked" in reqt:
                    mb = await bodyiq.tgbot.get_me()
                    mb = mb.username
                    await bodyiq.tgbot.send_message(BOTLOG_CHATID, "** البوت الخاص بك @{mb} مطرود من قناة الاشتراك الاجباري اعد اضافته**")
                    return
                if "not found" in reqt:
                    try:
                        c = await bodyiq.get_entity(ch)
                        chn = c.username
                        if c.username == None:
                            ra = await bodyiq.tgbot(ExportChatInviteRequest(ch))
                            chn = ra.link
                        ...
                        await bodyiq.tgbot.send_message(BOTLOG_CHATID, f"حدث خطا \n{er}")
                if "left" in reqt:
                    try:
                        c = await bodyiq.get_entity(ch)
                        chn = c.username
                ...
                if "error_code" in reqt:
                    await bodyiq.tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ غير معروف قم باعادة توجيه الرسالة ل@lMl10l لحل المشكلة\n{reqt}**")
