...
from bodython import BOTLOG_CHATID
from ..Config import Config
from ..helpers.utils import _format
from ..sql_helper.globals import addgvar, delgvar, gvarstatus
from . import AUTONAME, DEFAULT_GROUP, DEFAULT_BIO, edit_delete, bodyiq, logging
...
autopic_path = os.path.join(os.getcwd(), "bodython", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "bodython", "digital_pic.png")
digital_group_pic_path = os.path.join(os.getcwd(), "bodython", "digital_group_pic.png")
autophoto_path = os.path.join(os.getcwd(), "bodython", "photo_pfp.png")
auto_group_photo_path = os.path.join(os.getcwd(), "bodython", "photo_pfp.png")
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        body = gvarstatus("DEFAULT_PIC") or "bodython/helpers/styles/PaybAck.ttf"
        fnt = ImageFont.truetype(body, 65)
        drawn_text.text((200, 200), current_time, font=fnt, fill=colo)
        img.save(autophoto_path)
        file = await bodyiq.upload_file(autophoto_path)
        try:
            if i > 0:
                await bodyiq(
                    functions.photos.DeletePhotosRequest(
                        await bodyiq.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await bodyiq(functions.photos.UploadProfilePhotoRequest(file))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await bodyiq(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTOBIOSTART = gvarstatus("autobio") == "true"
...
@bodyiq.on(admin_cmd(pattern=f"{phow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@bodyiq.on(admin_cmd(pattern=f"{namew8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@bodyiq.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
bodyiq.loop.create_task(digitalpicloop())
bodyiq.loop.create_task(autoname_loop())
bodyiq.loop.create_task(autobio_loop())
