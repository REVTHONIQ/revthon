...
@bodyiq.ar_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@bodyiq.ar_cmd(pattern="توب الفلوس(.*)")
...
@bodyiq.ar_cmd(pattern="مسح حسابي(.*)")
...
@bodyiq.on(admin_cmd(pattern="(فلوسي|اموالي) ?(.*)"))
async def a(message):
    me = await message.client.get_me()
...
@bodyiq.on(admin_cmd(pattern="(بنكي|مصرفي) ?(.*)"))
...
@bodyiq.ar_cmd(func=lambda m:"راتب")
...
- مثال: .انشاء حساب الرافدين او بودي الاسلامي
1- .استثمار (مبلغ) 
- مثال : استثمار 18276
2- .حظ (المبلغ)
- مثال : حظ 17267
...
async def thief(message):
    ...
    be = int(accu.balance)
    body = int(be) - int(rt)
    update_bank(user.id, body)
    bodython = mee.first_name.replace("\u2060", "") if mee.first_name else mee.username
    ga = int(rt) + int(ppe)
    update_bank(mee.id, ga)
    await bodyiq.send_file(
                message.chat_id,
                "https://telegra.ph/file/9c4007ca621cc01a3c650.jpg",
                caption=f"سرق [{bodython}](tg://user?id={mee.id}) من [{user.first_name}](tg://user?id={user.id})\n المبلغ: {rt} 💵",
                )
...
@bodyiq.ar_cmd(pattern="انشاء حساب (.*)")
async def bankar(message):
    ...
    if input == "بودي الاسلامي":
        bankn = "مصرف بودي الاسلامي"
    elif input == "الرافدين":
    	bankn = "مصرف الرافدين"
    elif input != "الرافدين" or "بودي الاسلامي":
