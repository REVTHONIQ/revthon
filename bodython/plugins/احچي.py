...
@bodyiq.ar_cmd(pattern="احجي(?:\s|$)([\s\S]*)",
...
async def _(event):
    ...
    bodyevent = await edit_or_reply(event, "`يجري تنزيل الملف...`")
    oggfi = await event.client.download_media(reply, Config.TEMP_DIR)
    await bodyevent.edit("`يجري تحويل الكلام الى نص....`")
    r = sr.Recognizer()
    ...
    await bodyevent.edit(string_to_show)
...
@bodyiq.ar_cmd(pattern="استخرج(?:\s|$)([\s\S]*)",
...
async def _(event):
    ...
    pic_file = await bodyiq.download_media(reply, Config.TMP_DOWNLOAD_DIRECTORY)
    if not pic_file:
