...
@bodyiq.ar_cmd(
    ...
    info={
        "header": "To list all plugins in bodython.",
        "usage": "{tr}plugins",
    },
)
async def _(event):
    "To list all plugins in bodython"
    cmd = "ls bodython/plugins"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = f"**[بودي](tg://need_update_for_some_feature/) الـمـلفـات:**\n{o}"
...
@bodyiq.ar_cmd(
    ...
    info={
        "header": "To list all environment values in bodython.",
        "description": "to show all heroku vars/Config values in your bodython",
        "usage": "{tr}env",
    },
)
async def _(event):
    "To show all config values in bodython"
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (
        f"**[بودي](tg://need_update_for_some_feature/) قـائمـة الـفـارات:**\n\n\n{o}\n\n**انتبه هنالك معلومات حساسة لا تُعطِها لشخص غير موثوق**"
    )
    ...
    await bodyiq.send_message("me", OUTPUT)
...
@bodyiq.ar_cmd(pattern="رابط مباشر")
