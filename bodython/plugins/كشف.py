...
async def fetch_info(replied_user, event):
    ...
    rotbat = "⌁ من مطورين السورس 𓄂𓆃 ⌁" if user_id == 5699453147 else ("⌁ العضـو 𓅫 ⌁")
    rotbat = "⌁ مـالك الحساب 𓀫 ⌁" if user_id == (await event.client.get_me()).id and user_id != 5699453147  else rotbat
    caption = "✛━━━━━━━━━━━━━✛\n"
    caption += f"<b> {body_EM}╎الاسـم    ⇠ </b> {full_name}\n"
    caption += f"<b> {body_EM}╎المعـرف  ⇠ </b> {username}\n"
    caption += f"<b> {body_EM}╎الايـدي   ⇠ </b> <code>{user_id}</code>\n"
    caption += f"<b> {body_EM}╎الرتبـــه  ⇠ {rotbat} </b>\n"
    caption += f"<b> {body_EM}╎الصـور   ⇠ </b> {replied_user_profile_photos_count}\n"
    caption += f"<b> {body_EM}╎الحساب ⇠ </b> "
    caption += f'<a href="tg://user?id={user_id}">{first_name}</a>'
    caption += f"\n<b> {body_EM}╎البايـو    ⇠ </b> {user_bio} \n"
    caption += f"✛━━━━━━━━━━━━━✛"
