...
from body import CMD_HELP, LOAD_PLUG
...
LOGS = logging.getLogger("سورس بودي")
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"body/plugins/{shortname}.py")
        checkplugins(path)
        name = f"body.plugins.{shortname}"
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"body.plugins.{shortname}"] = mod
        LOGS.info(f"تم تحميل ملف {shortname}")
...
def remove_plugin(shortname):
    ...
    try:
        name = f"body.plugins.{shortname}"
        for i in reversed(range(len(body._event_builders))):
            ev, cb = body._event_builders[i]
            if cb.__module__ == name:
                del body._event_builders[i]
