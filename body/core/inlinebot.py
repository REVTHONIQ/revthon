...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**body"):
            ...
            builder = event.builder
            troll = os.path.join("./body", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./body", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./body", "hide.txt")
            try:
            ...
            result = builder.article(
                title="body Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب بودي **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
...
@body.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@body.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@body.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@body.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
