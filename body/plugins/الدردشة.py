...
async def ban_user(chat_id, i, rights):
    try:
        await body(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@body.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@body.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
@body.on(events.NewMessage(outgoing=False, pattern="/roz"))
async def _(event):
...
@body.ar_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    ...
    try:
        await body(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in body.iter_participants(chat_id):
        ...
        await body.send_message(chat_id, usrtxt)
...
@body.ar_cmd(pattern="الغاء التفليش", groups_only=True)
