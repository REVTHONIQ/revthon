...
from . import BOTLOG, BOTLOG_CHATID, body
...
@body.ar_cmd(pattern="(ت(ل)?ك(راف)?) ?(m|t|ميديا|نص)(?:\s|$)([\s\S]*)")
async def _(event):
    bodyevent = await edit_or_reply(event, "** ⌔∮ جار انشاء رابط تلكراف**")
    if BOTLOG:
    ...
    if input_str in ["ميديا", "m"]:
        ...
        await bodyevent.edit(f"**⌔∮ تم التحميل الى {downloaded_file_name}**")
        if downloaded_file_name.endswith((".webp")):
            resize_image(downloaded_file_name)
        try:
            media_urls = upload_file(downloaded_file_name)
        except exceptions.TelegraphException as exc:
            await bodyevent.edit(f"** ⌔∮ خطأ : **\n**{exc}**")
            os.remove(downloaded_file_name)
        else:
            end = datetime.now()
            ms = (end - start).seconds
            os.remove(downloaded_file_name)
            await bodyevent.edit(
        ...
        ms = (end - start).seconds
        body = f"https://graph.org/{response['path']}"
        await bodyevent.edit(
            f"**الرابط**:  [اضغط هنا]({body})\
