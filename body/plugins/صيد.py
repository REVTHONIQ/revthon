...
@body.ar_cmd(pattern="الصيد")
async def _(event):
    await event.edit(
        """
أوامر الصيد الخاصة بسورس بودي : 
...
async def _(event):
    ...
    isclaim.clear()
    isclaim.append("off")
    await body.send_message(event.chat_id, "**- تم الانتهاء من التثبيت بنجاح**")


@body.ar_cmd(pattern="حالة الصيد")
async def _(event):
    if "on" in isclaim:
...
@body.ar_cmd(pattern="حالة التثبيت")
async def _(event):
    if "on" in isauto:
