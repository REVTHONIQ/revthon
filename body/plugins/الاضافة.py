...
@body.ar_cmd(pattern="انضمام ([\s\S]*)")
async def lol(event):
    a = event.text
    bol = a[5:]
    ...
    try:
        await body(functions.channels.JoinChannelRequest(bol))
        await event.edit("**- تم الانضمام بنجاح  ✓**")
    except Exception as e:
        await event.edit(str(e))


@body.ar_cmd(pattern="اضافه ([\s\S]*)")
async def _(event):
...
@body.ar_cmd(pattern="ضيف ([\s\S]*)", groups_only=True)
async def get_users(event):
    legen_ = event.text[10:]
    body_chat = legen_.lower
    restricted = ["@super_jmthon", "@jmthon_support"]
    body = await edit_or_reply(event, f"**جارِ اضأفه الاعضاء من  ** {legen_}")
    if body_chat in restricted:
    ...
    if not sender.id == me.id:
        await body.edit("**▾∮ تتم العملية انتظر قليلا ...**")
    else:
        await body.edit("**▾∮ تتم العملية انتظر قليلا ...**")
    if event.is_private:
        return await body.edit("- لا يمكنك اضافه الاعضاء هنا")
    s = 0
    f = 0
    error = "None"
    await body.edit("**▾∮ حالة الأضافة:**\n\n**▾∮ تتم جمع معلومات المستخدمين 🔄 ...⏣**")
    async for user in event.client.iter_participants(event.pattern_match.group(1)):
        try:
            ...
            tol = f"@{user.username}"
            lol = tol.split("`")
            await body(InviteToChannelRequest(channel=event.chat_id, users=lol))
            s = s + 1
            await body.edit(
                f"**▾∮تتم الأضافة **\n\n• اضيف `{s}` \n•  خطأ بأضافة `{f}` \n\n**× اخر خطأ:** `{error}`"
            )
