...
from body import HEROKU_APP, UPSTREAM_REPO_URL, body
...
@body.ar_cmd(pattern="تحديث(| الان)?$")
async def upstream(event):
    ...
    try:
        txt = "فشل في التحديث لسورس بودي " + "**• حدث خطأ ما :**\n"
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"**• ارسل** `{cmdhd}تحديث التنصيب` لتحديث سورس بودي"
        )

    if force_update:
        await event.edit("**• جار التحديث الاجباري الى اخر اصدار انتظر قليلا**")
    if conf == "الان":
        await event.edit("**• جار تحديث سورس بودي أنتظر قليلا**")
