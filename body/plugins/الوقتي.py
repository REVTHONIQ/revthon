...
from . import edit_delete, body, logging
...
autopic_path = os.path.join(os.getcwd(), "body", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "body", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "body", "photo_pfp.png")
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await body.upload_file(autophoto_path)
        try:
            if i > 0:
                await body(
                    functions.photos.DeletePhotosRequest(
                        await body.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await body(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await body(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    while AUTOBIOSTART := gvarstatus("autobio") == "true":
        ...
        LOGS.info(bio)
        try:
            await body(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
@body.ar_cmd(pattern="الصورة الوقتية$")
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
...
body.loop.create_task(digitalpicloop())
body.loop.create_task(autoname_loop())
body.loop.create_task(autobio_loop())
