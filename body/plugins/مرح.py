...
from body.core.managers import edit_or_reply
from body.helpers import get_user_from_event
from razan.strings.fun import *

from . import *


@body.ar_cmd(pattern="رفع بكلبي(?: |$)(.*)")
...
@body.ar_cmd(pattern="نسبة الانوثة(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@body.ar_cmd(pattern="نسبة الغباء(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@body.ar_cmd(pattern="نسبة الرجولة(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@body.on(admin_cmd(pattern="نزوج(?:\s|$)([\s\S]*)"))
...
@body.on(admin_cmd(pattern="طلاك(?:\s|$)([\s\S]*)"))
async def mention(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
