...
@body.ar_cmd(pattern="كروباتي$")
async def oeo(event):
    result = await body(functions.channels.GetGroupsForDiscussionRequest())
    alist = []
    for item in result.chats:
        ...
        alist.append(roz)
    if alist:
        await body.send_message("me", "\n".join(alist))


@body.ar_cmd(pattern="الحاظرهم$")
async def main(event):
    result = await body(functions.contacts.GetBlockedRequest(offset=0, limit=1000000))
    alist = []
...
@body.ar_cmd(pattern="نوعه (.*)")
async def se(event):
    exe = event.text[5:]
    x = await body.get_entity(exe)
...
@body.ar_cmd(pattern="احذف (.*)")
async def se(event):
    exe = event.text[5:]
    await body.get_dialogs()
    chat = exe
    await body.delete_dialog(chat, revoke=True)
