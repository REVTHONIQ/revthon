...
@reviq.ar_cmd(func=lambda m:'reda')
async def tiktok_dl(message):
    ms = message.text
    #await reviq.send_message("@WKKKN", f"{message.sender}")
    if message.sender is None or message.sender.id == Config.OWNER_ID or message.sender.id in Config.SUDO_USERS:
        ...
        if ".تك https://vm.tiktok.com/" in ms:
            ...
            a = await reviq.send_message(message.chat.id, 'يجري البحث عن الملف..', parse_mode='md')
    ...
    #Using the default one can stop working any moment 
            ...
            await reviq.send_message(event.chat_id, str(r))
