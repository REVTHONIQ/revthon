...
from revthon import BOTLOG_CHATID
from ..Config import Config
from ..helpers.utils import _format
from ..sql_helper.globals import addgvar, delgvar, gvarstatus
from . import AUTONAME, DEFAULT_GROUP, DEFAULT_BIO, edit_delete, reviq, logging
...
autopic_path = os.path.join(os.getcwd(), "revthon", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "revthon", "digital_pic.png")
digital_group_pic_path = os.path.join(os.getcwd(), "revthon", "digital_group_pic.png")
autophoto_path = os.path.join(os.getcwd(), "revthon", "photo_pfp.png")
auto_group_photo_path = os.path.join(os.getcwd(), "revthon", "photo_pfp.png")
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        rev = gvarstatus("DEFAULT_PIC") or "revthon/helpers/styles/PaybAck.ttf"
        fnt = ImageFont.truetype(rev, 65)
        drawn_text.text((200, 200), current_time, font=fnt, fill=colo)
        img.save(autophoto_path)
        file = await reviq.upload_file(autophoto_path)
        try:
            if i > 0:
                await reviq(
                    functions.photos.DeletePhotosRequest(
                        await reviq.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await reviq(functions.photos.UploadProfilePhotoRequest(file))
    ...
    while AUTONAMESTART:
        ...
        for normal in HM:
            if normal in normzltext:
                ...
                DEVREVV = gvarstatus("TIME_REV") or ""
        name = f"{DEVREVV} {HM}"
        LOGS.info(name)
        try:
            await reviq(functions.account.UpdateProfileRequest(last_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await reviq(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTOBIOSTART = gvarstatus("autobio") == "true"
...
@reviq.on(admin_cmd(pattern=f"{phow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@reviq.on(admin_cmd(pattern=f"{namew8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@reviq.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
reviq.loop.create_task(digitalpicloop())
reviq.loop.create_task(autoname_loop())
reviq.loop.create_task(autobio_loop())
