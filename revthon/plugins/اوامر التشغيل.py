...
async def _(event):
    "Restarts the bot !!"
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "**⌔︙ريف ↻** \n" "**᯽︙ تم اعادة تشغيل السورس بنجاح ✅ ↻**")
    DEVREVV = await edit_or_reply(event, "᯽︙ سيتم اعادة التشغيل انتظر ")
    await event.edit("0%\n▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒")
    ...
    try:
        ...
        LOGS.error(e)
    try:
        add_to_collectionlist("restart_update", [DEVREVV.chat_id, DEVREVV.id])
    except Exception as e:
        LOGS.error(e)
    try:
        delgvar("ipaddress")
