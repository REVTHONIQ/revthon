...
@reviq.ar_cmd(
    ...
    info={
        "header": "To list all plugins in revthon.",
        "usage": "{tr}plugins",
    },
)
async def _(event):
    "To list all plugins in revthon"
    cmd = "ls revthon/plugins"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = f"**[ريف](tg://need_update_for_some_feature/) الـمـلفـات:**\n{o}"
...
@reviq.ar_cmd(
    ...
    info={
        "header": "To list all environment values in revthon.",
        "description": "to show all heroku vars/Config values in your revthon",
        "usage": "{tr}env",
    },
)
async def _(event):
    "To show all config values in revthon"
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (
        f"**[ريف](tg://need_update_for_some_feature/) قـائمـة الـفـارات:**\n\n\n{o}\n\n**انتبه هنالك معلومات حساسة لا تُعطِها لشخص غير موثوق**"
    )
    ...
    await reviq.send_message("me", OUTPUT)
...
@reviq.ar_cmd(pattern="رابط مباشر")
