...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"Revthon_{userid}_{pack}_anim"
    elif is_video:
        return f"Revthon_{userid}_{pack}_vid"
    return f"Revthon_{userid}_{pack}"
...
@reviq.on(admin_cmd(pattern="حزمة"))
async def revthonpkg(_):
    Rev = await _.get_reply_message()
    if not Rev:
    ...
    _id = Rev.media.document.attributes[1].stickerset.id
    _hash = Rev.media.document.attributes[1].stickerset.access_hash
    ...
    await edit_or_reply(
        _, f"**- تم اخذ الحزمه بنجاح ✓ \nالحزمه  → [اضغط هنا](https://t.me/addstickers/{revthon_Rev.set.short_name})**")
