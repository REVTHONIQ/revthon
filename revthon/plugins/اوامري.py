...
from RevIQ.razan.resources.assistant import *
from RevIQ.razan.resources.mybot import *
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("اوامري") and event.query.user_id == bot.uid:
            buttons = [
                [Button.inline("♰ اوامر الادمن ♰", data="reviq0")],
                [
            ...
            if REV_IC and REV_IC.endswith((".jpg", ".png", "gif", "mp4")):
                result = builder.photo(
                    REV_IC, text=ROE, buttons=buttons, link_preview=False
                )
            elif REV_IC:
...
async def repo(event):
    if event.fwd_from:
        return
    DEVREVV = Config.TG_BOT_USERNAME
    if event.reply_to_msg_id:
        await event.get_reply_message()
    response = await bot.inline_query(DEVREVV, "اوامري")
    await response[0].click(event.chat_id)
    await event.delete()


@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"reviq0")))
@check_owner
async def _(event):
    buttons = [
    [
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"jrzst")))
@check_owner
async def _(event):
    butze = [
    [
     Button.inline("التالي", data="tslrzj"),
     Button.inline("رجوع", data="reviq0")]]
    await event.edit(GRTSTI, buttons=butze)


@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"tslrzj")))
@check_owner
async def _(event):
    buttons = [
    [
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"krrznd")))
@check_owner
async def _(event):
    buttons = [
    [
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"rozbot")))
@check_owner
async def _(event):
    buttons = [
    [
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"Jmrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"r7brz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"sejrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"gro")))
@check_owner
async def _(event):
    buttons = [
    [
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"grrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"iiers")))
@check_owner
async def _(event):
    buttons = [
    [
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"rfhrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"uscuxrz")))
@check_owner
async def _(event):
    buttons = [[Button.inline("رجوع", data="reviq0"),]]
