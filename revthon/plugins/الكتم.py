...
@reviq.on(admin_cmd(pattern=f"كتم(?:\s|$)([\s\S]*)"))
async def muterev(event):
    if event.is_private:
...
@reviq.on(admin_cmd(pattern=f"الغاء كتم(?:\s|$)([\s\S]*)"))
async def unmuterev(event):
    if event.is_private:
        replied_user = await event.client.get_entity(event.chat_id)
