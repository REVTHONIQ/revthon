...
@reviq.on(admin_cmd(pattern="كتابة(?: |$)(.*)"))
async def _(event):
...
@reviq.on(admin_cmd(pattern="صوتية(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@reviq.on(admin_cmd(pattern="فيد(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@reviq.on(admin_cmd(pattern="لعبة(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@reviq.on(admin_cmd(pattern="الرابط$"))
async def _(e):
...
@reviq.on(admin_cmd(pattern="للكل تاك$"))
async def listall(revthon):
    if revthon.fwd_from:
        return
    ...
    await revthon.reply(mentions)
    await revthon.delete()
...
@reviq.on(admin_cmd(pattern=r"سبونج"))
async def kerz(kerz):
    await kerz.edit(R)
...
@reviq.on(admin_cmd(pattern=r"كلب"))
async def dog(dog):
    await dog.edit(M)
Z = (
    "┈┏━╮╭━┓┈╭━━━━╮\n"
...
@reviq.on(admin_cmd(pattern=r"ذئب"))
async def fox(fox):
    await fox.edit(H)


@reviq.on(admin_cmd(pattern=r"فيل"))
async def elephant(elephant):
    await elephant.edit(A)


@reviq.on(admin_cmd(pattern=r"هومر"))
async def homer(homer):
    await homer.edit(N)


@reviq.on(admin_cmd(pattern=r"بك"))
async def pig(pig):
    await pig.edit(Z)
