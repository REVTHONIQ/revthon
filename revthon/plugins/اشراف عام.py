#    جميع الحقوق محفوظة كتابة وتعديل  :   @DEVREVV
...
from revthon import CMD_HELP, reviq
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @RevThon", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@reviq.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(revthon):
    dc = razan = revthon
    i = 0
    sender = await dc.get_sender()
    me = await revthon.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await revthon.get_chat()
    if revthon.is_private:
        user = revthon.chat
        rank = revthon.pattern_match.group(1)
    else:
        revthon.chat.title
    try:
        user, rank = await get_full_user(revthon)
    except:
        pass
    if me == user:
       reviq = await razan.edit("▾∮ لا استطيع رفع نفسي 🧸🤍،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await revthon.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await revthon.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
...
@reviq.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(revthon):
    dc = razan = revthon
    i = 0
    sender = await dc.get_sender()
    ...
    try:
        ...
        pass
    if me == user:
       reviq = await razan.edit("▾∮ لا استطيع تنزيل نفسي 🧸🤍")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
