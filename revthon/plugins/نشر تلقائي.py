...
@reviq.on(admin_cmd(pattern="(نشر_تلقائي|النشر_التلقائي)"))
async def _(event):
...
@reviq.on(admin_cmd(pattern="(ايقاف_نشر|ايقاف_النشر)"))
async def _(event):
    if (event.is_private or event.is_group):
        return await edit_or_reply(event, "**᯽︙ عـذراً .. النشر التلقائي خـاص بالقنـوات فقـط**")
    jok = event.pattern_match.group(1)
...
@reviq.ar_cmd(incoming=True, forword=None)
async def _(event):
    if event.is_private:
        return
    ...
    for chat in channels_set:
        if event.media:
            ...
            await reviq.send_message(int(chat), event.message)
