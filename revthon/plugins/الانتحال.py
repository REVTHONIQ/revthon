...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "الحمد لله دائماً وابداً 🎀 𝑆𝐻 : @REVTHON"
)


@reviq.ar_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
    mid = await reviq.get_me()
...
#Reda
revs = ["revthon", "revthonsupport"]
@reviq.ar_cmd(pattern="انتحال_الدردشه")
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        chat_id = msg
        try:
            result = await reviq(GetFullChannelRequest(
                chat_id
            ))
        except ValueError:
        ...
        mych = await reviq(GetFullChannelRequest(
                event.chat_id
            ))
        if msg in revs:
        ...
        try:
            await reviq(functions.channels.EditTitleRequest(
                channel=await reviq.get_entity(event.chat_id),
        ...
        try:
            await reviq(functions.messages.EditChatAboutRequest(
            peer=event.chat_id,
            about=result.full_chat.about
        ))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        profile_pic = await reviq.download_profile_photo(chat_id, Config.TEMP_DIR)
        pfile = await reviq.upload_file(profile_pic)
        try:
            await reviq(functions.channels.EditPhotoRequest(event.chat_id, pfile))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        ...
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @revthon")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
...
#Reda
@reviq.ar_cmd(pattern="اعادة_الدردشه")
async def reda_back(event):
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            ...
            async for photo in reviq.iter_profile_photos(event.chat_id, limit=1) :
                    await reviq(
