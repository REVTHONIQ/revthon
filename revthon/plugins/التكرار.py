...
async def spam_function(event, revthon, reviq, sleeptimem, sleeptimet, DelaySpam=False):

    counter = int(reviq[0])
    if len(reviq) == 2:
        spam_message = str(reviq[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            if event.reply_to_msg_id:
                await revthon.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and revthon.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            revthon = await event.client.send_file(
                event.chat_id, revthon, caption=revthon.text
            )
            await _catutils.unsavegif(event, revthon)
        ...
        if BOTLOG:
            ...
            revthon = await event.client.send_file(BOTLOG_CHATID, revthon)
            await _catutils.unsavegif(event, revthon)
        return
    elif event.reply_to_msg_id and revthon.text:
        spam_message = revthon.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
@reviq.ar_cmd(pattern="كرر (.*)")
async def spammer(event):
    revthon = await event.get_reply_message()
    reviq = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    try:
        counter = int(reviq[0])
    except Exception:
    ...
    await spam_function(event, revthon, reviq, sleeptimem, sleeptimet)

@reviq.on(admin_cmd(pattern="(مكرر|مؤقت|موقت)"))
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, reviq, sleeptimem, sleeptimet, DelaySpam=True)


@reviq.ar_cmd(pattern="تكرار الملصق$")
...
@reviq.ar_cmd(pattern="ايقاف التكرار ?(.*)")
