...
@reviq.ar_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@reviq.ar_cmd(pattern="توب الفلوس(.*)")
...
@reviq.ar_cmd(pattern="مسح حسابي(.*)")
...
@reviq.on(admin_cmd(pattern="(فلوسي|اموالي) ?(.*)"))
async def a(message):
    me = await message.client.get_me()
...
@reviq.on(admin_cmd(pattern="(بنكي|مصرفي) ?(.*)"))
...
@reviq.ar_cmd(func=lambda m:"راتب")
...
- مثال: .انشاء حساب الرافدين او ريف الاسلامي
1- .استثمار (مبلغ) 
- مثال : استثمار 18276
2- .حظ (المبلغ)
- مثال : حظ 17267
...
async def thief(message):
    ...
    be = int(accu.balance)
    rev = int(be) - int(rt)
    update_bank(user.id, rev)
    revthon = mee.first_name.replace("\u2060", "") if mee.first_name else mee.username
    ga = int(rt) + int(ppe)
    update_bank(mee.id, ga)
    await reviq.send_file(
                message.chat_id,
                "https://telegra.ph/file/9c4007ca621cc01a3c650.jpg",
                caption=f"سرق [{revthon}](tg://user?id={mee.id}) من [{user.first_name}](tg://user?id={user.id})\n المبلغ: {rt} 💵",
                )
...
@reviq.ar_cmd(pattern="انشاء حساب (.*)")
async def bankar(message):
    ...
    if input == "ريف الاسلامي":
        bankn = "مصرف ريف الاسلامي"
    elif input == "الرافدين":
    	bankn = "مصرف الرافدين"
    elif input != "الرافدين" or "ريف الاسلامي":
