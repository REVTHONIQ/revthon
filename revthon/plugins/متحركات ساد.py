...
#جميع الحقوق محفوظة لسـورس ريف تخـمط تبيـن فشلـك
...
async def tmgif(event):
    if event.fwd_from:
        return
    DEVREVV = await reply_id(event)
    if sad:
        reviq = f"**˛ REV ، ٰ𝖦𝗂𝖿 𝖲ِ𝖺ٰ𝖣 . .**\n"
        reviq += f"✛━━━━━━━━━━━━━✛\n"
        reviq += f"**الـمتحـرڪـة الأولـى **"
        await event.client.send_file(event.chat_id, sad, caption=reviq, reply_to=DEVREVV)
