...
def main_menu():
    text = f"**- [سـورس ريف](https://t.me/RevThon)\
        \n المسـاعد\
        \n\
        \n◽ ريف لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("reviq"):
            buttons = [
                (
                    Button.inline("الحـالة ⚒️", data="stats"),
                    Button.url(" Revthon UsᴇʀBoᴛ", "https://t.me/Revthon"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./revthon", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس ريف 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./revthon", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس ريف 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./revthon", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس ريف 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© revthon Help",
                description="Help menu for revthon",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/Revthon"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/Revthon1",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت ريف 🧸♥](https://t.me/Revthon)**\
            \n\
            \n❤ ريف هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/Revthon1)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@reviq.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
